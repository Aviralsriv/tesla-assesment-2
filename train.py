...
class SSLTrainer:
    ...
    def _train_step(self, batch: dict) -> dict:
        ...
        with autocast(enabled=self.cfg.amp):
            ...
            with torch.no_grad():
                z1, z2 = out1['fused'], out2['fused']
                N      = z1.size(0)
                sim    = torch.mm(z1, z2.T) # (N, N)
                # For each z1, the positive match is at the same index in z2
...
def parse_args():
    ...
    p.add_argument('--freeze_dino',  action='store_true', default=False)
